...
await test('All 319 static prompt files exist on disk with valid structure', async () => {
  const files = fs.readdirSync('prompts').filter(f => f.endsWith('.html'));
  assert(files.length === 319, `Expected 319 files, found ${files.length}`);
...
await test('Zero broken internal href/src links across all HTML files', async () => {
  ...
  assert(allHtml.length >= 328, `Expected at least 328 HTML files, found ${allHtml.length}`);
...
await test('All 319 prompts in prompts.json have full text, title, and valid categories', async () => {
  const prompts = JSON.parse(fs.readFileSync('data/prompts.json', 'utf8'));
  assert(prompts.length === 319, `Expected 319 items in json, found ${prompts.length}`);
  for (const p of prompts) {
...
await test('browse.html contains full search index, General category pill, and ID search support', async () => {
  ...
  assert(html.includes('General (100)'), 'Missing General (100) category pill');
...
await test('data/database.sqlite exists and contains all prompts, categories, and media', async () => {
  ...
  assert(stats.totalPrompts === 319, `Expected 319 prompts in DB, got ${stats.totalPrompts}`);
  assert(stats.totalCategories === 14, `Expected 14 categories in DB, got ${stats.totalCategories}`);
  assert(stats.totalMedia >= 392, `Expected at least 392 media records, got ${stats.totalMedia}`);
...
await test('REST API endpoints /api/stats, /api/categories, /api/prompts, and /api/prompts/18', async () => {
  ...
  assert(statsRes.totalPrompts === 319, 'API /api/stats totalPrompts is not 319');
  ...
  assert(promptsRes.totalItems === 319, 'API /api/prompts totalItems is not 319');
...
// 13. Test CSV Export
await test('data/prompts.csv exists and contains 319 exported prompt records', async () => {
  ...
  // rowCount counts the header row + 319 prompt records = 319 boundaries
  assert(rowCount === 319, `Expected 319 data rows in CSV, got ${rowCount}`);
});
